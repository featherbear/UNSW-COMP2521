...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
	...
	for (VList n = g->connections[start]; n != NULL; n = n->next) {
	    if (n->v == end) {
	        type[itemCount++] = n->type;
	    }

	    // Sea links have two graph edges
	    // We could use `connections()` here again, but I can't be bothered to malloc space for type
	    else if (n->type == BOAT) {
            for (VList o = g->connections[n->v]; o != NULL; o = o->next) {
                if (o->v == end) {
                    type[itemCount++] = o->type;
                    break;
                }
            }
	    }
